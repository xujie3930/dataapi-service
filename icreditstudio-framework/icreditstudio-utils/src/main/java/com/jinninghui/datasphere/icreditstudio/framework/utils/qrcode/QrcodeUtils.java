 ...
 * 二维码生成/读取
 * 
 * mvn repository：ZXing Core » 3.3.1 参考：https://github.com/binarywang
 * 
 * @author lidab
 */
public abstract class QrcodeUtils {

	private static final int DEFAULT_SIZE = 400;// 生成二维码的默认边长，因为是正方形的，所以高度和宽度一致
	private static final String FORMAT = "jpg";// 生成二维码的格式

	/**
	 * 创建生成默认尺寸的二维码图片不带logo
	 *
	 * @param content
	 *            二维码内容
	 * @return 二维码图片的字节数组
	...
	public static byte[] createQrcode(String content) {
		return createQrcode(content, DEFAULT_SIZE, null);
	}

	/**
	 * 创建生成默认尺寸的二维码图片 可以指定是否带logo
	 *
	 * @param content
	 *            二维码内容
	 * @param logoFile
	...
	public static byte[] createQrcode(String content, File logoFile) {
		return createQrcode(content, DEFAULT_SIZE, logoFile);
	}
	 ...
	 * @return 二维码图片的字节数组
	 */
	public static byte[] createQrcode(String content, int size, File logoFile) {
		if (logoFile != null && !logoFile.exists()) {
			throw new IllegalArgumentException("请提供正确的logo文件！");
		}
		BitMatrix qrCodeMatrix = createQrcodeMatrix(content, size);
	 ...
	 *            图片格式
	 */
	protected static void overlapImage(BufferedImage image, String format, String imagePath, File logoFile,
